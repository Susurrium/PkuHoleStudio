...
3. 若测试在线帖子，后续页中带图片的评论也必须显示图片。
4. 给两个帖子分别设置不同本地标签，再给其中一个帖子同时设置这两个标签。
5. 回到“资料库”，选择一个标签，只应显示拥有该标签的帖子。
6. 同时选择两个标签，只应显示同时拥有两者的帖子。
7. 刷新浏览器，标签选择和结果应保持；地址栏应包含 `tag=` 参数。
8. 在“全文搜索”输入一个关键词并重复单标签、双标签测试；翻到下一页时标签条件不能丢失。
