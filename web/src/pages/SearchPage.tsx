...
export function SearchPage() {
  ...
  const cursor = Number(params.get('cursor') ?? 0)
	...
	const results = useQuery({ queryKey: ['search', q, cursor], queryFn: () => api.search({ q, cursor, limit: 20 }), enabled: Boolean(q) })
	const history = useQuery({ queryKey: ['search-history'], queryFn: api.searchHistory, enabled: !q })
  function submit(event: FormEvent) { event.preventDefault(); const value = draft.trim(); setParams(value ? { q: value } : {}) }
  return <>
    ...
    <div className="mt-6">
		{!q ? <><EmptyState title="从一个具体问题开始" description="例如“数据结构 作业量”或“#123456”。所有检索默认只在本地资料库中进行。" />{history.data?.length ? <div className="panel mt-5 p-5"><p className="eyebrow">RECENT QUERIES</p><div className="mt-3 flex flex-wrap gap-2">{history.data.map((item) => <button key={item.id} className="badge hover:border-teal hover:text-teal" onClick={() => { setDraft(item.query); setParams({ q: item.query }) }}>{item.query}</button>)}</div></div> : null}</> : results.isLoading ? <LoadingState label={`正在检索“${q}”…`} /> : results.error ? <ErrorState error={results.error} /> : results.data?.items.length ? <><p className="mb-4 text-sm text-ink-soft">当前页找到 {results.data.items.length} 个帖子结果</p><div className="grid gap-4 xl:grid-cols-2">{results.data.items.map((post) => <PostCard key={post.pid} post={post} />)}</div>{results.data.has_more && <div className="mt-6 flex justify-center"><button className="button-secondary" onClick={() => setParams({ q, cursor: String(results.data?.next_cursor ?? 0) })}>下一页</button></div>}</> : <EmptyState title="没有找到匹配内容" description="减少关键词、检查 PID，或同步更多资料后重试。" />}
    </div>
  </>
}
