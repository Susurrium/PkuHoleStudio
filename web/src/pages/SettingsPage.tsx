...
export function SettingsPage() {
  ...
  if (capabilities.error || !capabilities.data) return <ErrorState error={capabilities.error} />
  return <>
    <PageHeader eyebrow="LOCAL CONFIG" title="设置与能力" description="首版展示当前运行能力。数据库连接、同步账号和 AI 密钥仍通过本机配置文件管理，不会由网页回显敏感值。" />
    <div className="grid gap-5 lg:grid-cols-2">
      <SettingCard icon={Database} title="资料库" items={[['Schema', `v${capabilities.data.schema_version}`], ['全文搜索', capabilities.data.fts5 ? 'FTS5 trigram' : 'LIKE 兼容模式'], ['归档导入', capabilities.data.archive_import ? '可用' : '不可用']]} />
      <SettingCard icon={Server} title="本机服务" items={[['API', capabilities.data.api_version], ['任务管理', capabilities.data.jobs ? '持久化可用' : '不可用'], ['访问范围', '由启动参数决定']]} />
      <section className="panel p-6 lg:col-span-2"><div className="flex items-start gap-4"><div className="grid size-11 place-items-center rounded-xl bg-coral-soft text-coral"><Sparkles size={20} /></div><div className="flex-1"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-lg font-semibold">AI Provider</h2><p className="mt-1 text-sm text-ink-soft">OpenAI-compatible / DeepSeek</p></div><span className="badge gap-1"><CircleOff size={11} />尚未启用</span></div><p className="mt-5 rounded-xl border border-dashed border-line bg-paper/40 p-4 text-sm leading-6 text-ink-soft">AI 将在下一阶段启用。默认只使用本地 FTS 检索；实时树洞搜索会保持独立、默认关闭的开关。</p></div></div></section>
    </div>
  </>
}
