...
import { Activity, Check, CircleOff, Database, Pencil, Plus, Server, Sparkles, Tags, Trash2, X } from 'lucide-react'
...
function ProviderManager({ initial, refresh }: { initial: Settings; refresh: () => unknown }) {
	...
	const activate = useMutation({ mutationFn: api.activateAIProviderSetting, onSuccess: refresh })
	const remove = useMutation({ mutationFn: api.deleteAIProviderSetting, onSuccess: refresh })
	const test = useMutation({ mutationFn: api.testAIProviderSetting })
	return <section className="panel p-6 lg:col-span-2"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-lg font-semibold">Provider 列表</h2><p className="mt-1 text-xs text-ink-soft">活动 Provider 用于新会话；编辑和切换保存后立即生效，无需重启。</p></div><button className="button-secondary" onClick={() => setEditingID('new')}><Plus size={14} />新增 Provider</button></div><div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">{providers.map((item) => <div key={item.id} className={`rounded-xl border p-4 ${item.active ? 'border-teal bg-teal-soft/30' : 'border-line bg-white/40'}`}><div className="flex items-start justify-between gap-2"><div><p className="font-semibold">{item.name}</p><p className="mt-1 font-mono text-xs text-ink-soft">{item.model}</p></div>{item.active && <span className="badge !border-teal/30 !text-teal">运行中</span>}</div><p className="mt-3 truncate text-xs text-ink-soft" title={item.base_url}>{item.base_url}</p><p className="mt-2 text-xs text-ink-soft">API key：{item.api_key_configured ? '已配置' : '未配置（允许本地无鉴权服务）'}</p><div className="mt-4 flex flex-wrap gap-2">{!item.active && <button className="button-secondary !py-1.5" disabled={activate.isPending} onClick={() => activate.mutate(item.id)}>设为活动</button>}<button className="button-secondary !px-3 !py-1.5" onClick={() => setEditingID(item.id)}><Pencil size={13} />编辑</button><button className="button-secondary !px-3 !py-1.5" disabled={test.isPending} onClick={() => test.mutate(item.id)}><Activity size={13} />测试</button><button className="button-secondary !px-3 !py-1.5 !text-coral" disabled={remove.isPending || providers.length <= 1} onClick={() => remove.mutate(item.id)}><Trash2 size={13} /></button></div>{test.data?.provider_id === item.id && <p className="mt-3 text-xs text-teal">连接成功 · {test.data.latency_ms} ms</p>}</div>)}</div>{test.error && <p className="mt-3 text-xs text-coral">测试失败：{String(test.error)}</p>}{editingID && <ProviderEditor provider={editingID === 'new' ? undefined : providers.find((item) => item.id === editingID)} close={() => setEditingID(undefined)} refresh={refresh} />}</section>
}

function ProviderEditor({ provider, close, refresh }: { provider?: import('../lib/types').AIProviderSetting; close: () => void; refresh: () => unknown }) {
	...
	const save = useMutation({ mutationFn: () => provider ? api.updateAIProviderSetting(provider.id, { ...draft, api_key: draft.api_key || undefined }) : api.createAIProviderSetting({ ...draft, api_key: draft.api_key || undefined }), onSuccess: () => { refresh(); close() } })
	return <div className="mt-5 rounded-xl border border-line bg-paper/45 p-4"><div className="flex items-center justify-between"><h3 className="font-semibold">{provider ? `编辑 ${provider.name}` : '新增 OpenAI-compatible Provider'}</h3><button className="button-secondary !p-2" aria-label="关闭 Provider 编辑" onClick={close}><X size={14} /></button></div><div className="mt-3 grid gap-3 md:grid-cols-2"><label className="text-xs text-ink-soft">名称<input className="field mt-1" value={draft.name} onChange={(event) => setDraft({ ...draft, name: event.target.value })} /></label><label className="text-xs text-ink-soft">模型<input className="field mt-1" value={draft.model} onChange={(event) => setDraft({ ...draft, model: event.target.value })} /></label><label className="text-xs text-ink-soft md:col-span-2">Base URL<input className="field mt-1" value={draft.base_url} onChange={(event) => setDraft({ ...draft, base_url: event.target.value })} /></label><label className="text-xs text-ink-soft md:col-span-2">API key（留空保留已有密钥；本地服务可为空）<input className="field mt-1" type="password" autoComplete="new-password" value={draft.api_key} placeholder={provider?.api_key_configured ? '已配置；不会回显' : '未配置；本地无鉴权服务可留空'} onChange={(event) => setDraft({ ...draft, api_key: event.target.value })} /></label><label className="text-xs text-ink-soft">Temperature<input className="field mt-1" type="number" min="0" max="2" step="0.1" value={draft.temperature} onChange={(event) => setDraft({ ...draft, temperature: Number(event.target.value) })} /></label><label className="text-xs text-ink-soft">最大输出 tokens<input className="field mt-1" type="number" min="1" max="1000000" value={draft.max_output_tokens} onChange={(event) => setDraft({ ...draft, max_output_tokens: Number(event.target.value) })} /></label><label className="text-xs text-ink-soft">请求超时（秒）<input className="field mt-1" type="number" min="1" max="3600" value={draft.request_timeout_seconds} onChange={(event) => setDraft({ ...draft, request_timeout_seconds: Number(event.target.value) })} /></label></div><div className="mt-4 flex gap-2"><button className="button-primary" disabled={save.isPending || !draft.name.trim() || !draft.model.trim() || !draft.base_url.trim()} onClick={() => save.mutate()}>{save.isPending ? '正在保存…' : '保存并立即应用'}</button><button className="button-secondary" onClick={close}>取消</button></div>{save.error && <p className="mt-3 text-xs text-coral">{String(save.error)}</p>}</div>
}
...
function AISettingsForm({ initial, runtimeConfigured, refresh }: { initial: Settings; runtimeConfigured: boolean; refresh: () => unknown }) {
	...
	const save = useMutation({ mutationFn: () => api.updateSettings(draft), onSuccess: refresh })
	const number = (key: keyof SettingsUpdate, value: string) => setDraft((current) => ({ ...current, [key]: Number(value) }))
	return <section className="panel p-6 lg:col-span-2"><div className="flex items-start gap-4"><div className="grid size-11 place-items-center rounded-xl bg-coral-soft text-coral"><Sparkles size={20} /></div><div className="flex-1"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-lg font-semibold">AI 全局设置</h2><p className="mt-1 text-sm text-ink-soft">运行时：{initial.ai_runtime_provider || initial.ai_provider_name} · {initial.ai_runtime_model || initial.ai_model}</p></div><span className={`badge gap-1 ${runtimeConfigured ? '!border-teal/30 !bg-teal-soft !text-teal' : ''}`}>{runtimeConfigured ? <><Check size={11} />当前进程已启用</> : <><CircleOff size={11} />当前进程未启用</>}</span></div><div className="mt-5 grid gap-4 md:grid-cols-2"><label className="text-xs text-ink-soft">最大检索轮数<input className="field mt-1" type="number" min="1" max="5" value={draft.ai_max_search_rounds} onChange={(event) => number('ai_max_search_rounds', event.target.value)} /></label></div><div className="mt-4 flex flex-wrap gap-5 text-sm"><label className="inline-flex items-center gap-2"><input type="checkbox" checked={draft.ai_enabled} onChange={(event) => setDraft({ ...draft, ai_enabled: event.target.checked })} />启用 AI</label><label className="inline-flex items-center gap-2"><input type="checkbox" checked={draft.ai_live_search} onChange={(event) => setDraft({ ...draft, ai_live_search: event.target.checked })} />允许 AI 实时搜索树洞</label></div><div className="mt-5 flex flex-wrap items-center gap-3"><button className="button-primary" disabled={save.isPending} onClick={() => save.mutate()}>{save.isPending ? '正在保存…' : '保存并立即应用'}</button><p className="text-xs text-ink-soft">正在生成的回答使用原快照；新会话使用最新活动 Provider。</p></div>{save.isSuccess && <p className="mt-3 text-sm text-teal">设置已安全写入并应用，无需重启。</p>}{save.error && <p className="mt-3 text-sm text-coral">{String(save.error)}</p>}</div></div></section>
}
