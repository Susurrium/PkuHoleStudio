...
import { Check, CircleOff, Database, Pencil, Plus, Server, Sparkles, Tags, Trash2 } from 'lucide-react'
  ...
  return <>
    ...
    <div className="grid gap-5 lg:grid-cols-2">
      <SettingCard icon={Database} title="资料库" items={[['Schema', `v${capabilities.data.schema_version}`], ['全文搜索', capabilities.data.fts5 ? 'FTS5 trigram' : 'LIKE 兼容模式'], ['归档导入', capabilities.data.archive_import ? '可用' : '不可用'], ['归档导出', capabilities.data.archive_export ? '可用' : '不可用']]} />
		...
		<ProviderManager initial={settings.data} refresh={() => settings.refetch()} />
...
function ProviderManager({ initial, refresh }: { initial: Settings; refresh: () => unknown }) {
	...
	const create = useMutation({ mutationFn: () => api.createAIProviderSetting({ name, base_url: baseURL, model, temperature: 0.2, max_output_tokens: 4096, request_timeout_seconds: 120, api_key: apiKey || undefined }), onSuccess: () => { setAdding(false); setName(''); setAPIKey(''); refresh() } })
	...
	return <section className="panel p-6 lg:col-span-2"><div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-lg font-semibold">Provider 列表</h2><p className="mt-1 text-xs text-ink-soft">只有活动 Provider 会被 AI 会话使用。切换后需要重启应用。</p></div><button className="button-secondary" onClick={() => setAdding((value) => !value)}><Plus size={14} />新增 Provider</button></div><div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">{providers.map((item) => <div key={item.id} className={`rounded-xl border p-4 ${item.active ? 'border-teal bg-teal-soft/30' : 'border-line bg-white/40'}`}><div className="flex items-start justify-between gap-2"><div><p className="font-semibold">{item.name}</p><p className="mt-1 font-mono text-xs text-ink-soft">{item.model}</p></div>{item.active && <span className="badge !border-teal/30 !text-teal">活动</span>}</div><p className="mt-3 truncate text-xs text-ink-soft" title={item.base_url}>{item.base_url}</p><p className="mt-2 text-xs text-ink-soft">API key：{item.api_key_configured ? '已配置' : '未配置'}</p><div className="mt-4 flex gap-2">{!item.active && <button className="button-secondary !py-1.5" disabled={activate.isPending} onClick={() => activate.mutate(item.id)}>设为活动</button>}<button className="button-secondary !py-1.5 !text-coral" disabled={remove.isPending || providers.length <= 1} onClick={() => remove.mutate(item.id)}><Trash2 size={13} />删除</button></div></div>)}</div>{adding && <div className="mt-5 rounded-xl border border-line bg-paper/45 p-4"><h3 className="font-semibold">新增 OpenAI-compatible Provider</h3><div className="mt-3 grid gap-3 md:grid-cols-2"><input className="field" value={name} onChange={(event) => setName(event.target.value)} placeholder="名称，例如 OpenAI" /><input className="field" value={model} onChange={(event) => setModel(event.target.value)} placeholder="模型，例如 gpt-5-mini" /><input className="field md:col-span-2" value={baseURL} onChange={(event) => setBaseURL(event.target.value)} placeholder="https://api.example.com/v1" /><input className="field md:col-span-2" type="password" autoComplete="new-password" value={apiKey} onChange={(event) => setAPIKey(event.target.value)} placeholder="API key（只写入，不回显；本地服务可留空）" /></div><div className="mt-3 flex gap-2"><button className="button-primary" disabled={create.isPending || !name.trim() || !model.trim() || !baseURL.trim()} onClick={() => create.mutate()}>保存 Provider</button><button className="button-secondary" onClick={() => setAdding(false)}>取消</button></div>{(create.error || activate.error || remove.error) && <p className="mt-3 text-xs text-coral">{String(create.error || activate.error || remove.error)}</p>}</div>}</section>
}
