...
export function AIPage() {
  ...
  return <>
    ...
    {!configured && <div className="mb-6 rounded-2xl border border-coral/25 bg-coral-soft/45 p-5 text-sm leading-6"><p className="font-semibold text-coral">AI Provider 尚未配置</p><p className="mt-1 text-ink-soft">在本机 `data/config.json` 中启用 AI 并填写 API key，或设置 `PKUHOLE_AI_API_KEY` 环境变量后重启。DeepSeek 模板已内置。</p><Link to="/settings" className="mt-3 inline-flex font-semibold text-teal hover:underline">查看当前能力 →</Link></div>}
