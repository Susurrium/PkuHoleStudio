...
function AdvancedSync({ disabled, onCreate }: { disabled: boolean; onCreate: (type: string, payload: unknown) => void }) {
	...
	return <section className="mt-6"><div className="mb-4"><p className="eyebrow">ADVANCED CRAWLER</p><h2 className="mt-1 text-xl font-semibold">高级采集与修复</h2></div><div className="grid gap-5 xl:grid-cols-3"><form className="panel p-5" onSubmit={(event) => { event.preventDefault(); onCreate('sync_pages', { start_page: startPage, pages, options: { post_limit: 200, comment_limit: 200, save_json: saveJSON, fetch_images: false, convert_webp: convertWebP } }) }}><h3 className="font-semibold">顺序采集页面</h3><p className="mt-2 text-sm text-ink-soft">从指定页开始逐页采集，每页完成后保存 checkpoint。</p><div className="mt-4 grid grid-cols-2 gap-3"><label className="text-xs text-ink-soft">起始页<input className="field mt-1" type="number" min={1} value={startPage} onChange={(event) => setStartPage(Math.max(1, Number(event.target.value) || 1))} /></label><label className="text-xs text-ink-soft">页数<input className="field mt-1" type="number" min={1} max={50} value={pages} onChange={(event) => setPages(Math.max(1, Math.min(50, Number(event.target.value) || 1)))} /></label></div><label className="mt-3 inline-flex items-center gap-2 text-xs text-ink-soft"><input type="checkbox" checked={saveJSON} onChange={(event) => setSaveJSON(event.target.checked)} />缓存原始 API JSON，完成后可保存</label><button className="button-primary mt-4 w-full" disabled={disabled}>创建顺序采集任务</button></form><form className="panel p-5" onSubmit={(event) => { event.preventDefault(); onCreate('monitor_latest', { pages: monitorPages, interval_seconds: interval, options: { post_limit: 200, comment_limit: 200, save_json: saveJSON, convert_webp: convertWebP } }) }}><h3 className="font-semibold">持续监控最新页</h3><p className="mt-2 text-sm text-ink-soft">循环检查前 N 页。暂停或退出程序后不会自动恢复运行。</p><div className="mt-4 grid grid-cols-2 gap-3"><label className="text-xs text-ink-soft">监控页数<input className="field mt-1" type="number" min={1} max={50} value={monitorPages} onChange={(event) => setMonitorPages(Math.max(1, Math.min(50, Number(event.target.value) || 1)))} /></label><label className="text-xs text-ink-soft">间隔秒<input className="field mt-1" type="number" min={15} value={interval} onChange={(event) => setInterval(Math.max(15, Number(event.target.value) || 60))} /></label></div><button className="button-primary mt-4 w-full" disabled={disabled}>启动持续监控</button></form><div className="panel p-5"><h3 className="font-semibold">资料修复与原始数据</h3><p className="mt-2 text-sm text-ink-soft">补全媒体、缩略图和引用，或保存采集时缓存的原始响应。</p><div className="mt-4 grid grid-cols-2 gap-3"><input className="field" type="number" min={1} value={thumbStart} aria-label="缩略图起始 ID" onChange={(event) => setThumbStart(Math.max(1, Number(event.target.value) || 1))} /><input className="field" type="number" min={thumbStart} value={thumbEnd} aria-label="缩略图结束 ID" onChange={(event) => setThumbEnd(Math.max(thumbStart, Number(event.target.value) || thumbStart))} /></div><label className="mt-3 inline-flex items-center gap-2 text-xs text-ink-soft"><input type="checkbox" checked={convertWebP} onChange={(event) => setConvertWebP(event.target.checked)} />旧版图片采集转换为 WebP</label><div className="mt-3 grid gap-2"><button className="button-secondary" disabled={disabled} onClick={() => onCreate('repair_media', {})}>补全资料库缺失媒体</button><button className="button-secondary" disabled={disabled} onClick={() => onCreate('fetch_images', { convert_webp: convertWebP })}>执行旧版图片补全</button><button className="button-secondary" disabled={disabled || thumbEnd < thumbStart} onClick={() => onCreate('repair_thumbnails', { start_id: thumbStart, end_id: thumbEnd, convert_webp: convertWebP })}>补全缩略图范围</button><button className="button-secondary" disabled={disabled} onClick={() => onCreate('save_raw_json', {})}>保存已缓存原始 JSON</button><button className="button-secondary" disabled={disabled} onClick={() => onCreate('rebuild_references', {})}>重建引用关系</button><button className="button-secondary" disabled={disabled} onClick={() => onCreate('cleanup_staging', { retention_days: 7 })}>清理过期暂存文件</button></div></div></div></section>
}
...
function PIDSyncCard({ disabled, onSubmit }: { disabled: boolean; onSubmit: (pids: number[], includeComments: boolean, includeMedia: boolean) => void }) {
  ...
  return <form className="panel p-5" onSubmit={(event) => { event.preventDefault(); if (pids.length) onSubmit([...new Set(pids)], includeComments, includeMedia) }}><div className="grid size-10 place-items-center rounded-xl bg-coral-soft text-coral"><ListRestart size={19} /></div><h2 className="mt-5 text-lg font-semibold">同步指定 PID</h2><p className="mt-2 min-h-12 text-sm leading-6 text-ink-soft">抓取帖子，并可完整分页抓取评论和图片。输入多个 PID 时用空格或逗号分隔。</p><label className="mt-4 block text-xs font-medium text-ink-soft">PID 列表<textarea className="field mt-1.5 min-h-20 resize-y" value={value} onChange={(event) => setValue(event.target.value)} placeholder="1234567, 2345678" /></label><div className="mt-3 flex flex-wrap gap-4 text-xs text-ink-soft"><label className="inline-flex items-center gap-2"><input type="checkbox" checked={includeComments} onChange={(event) => setIncludeComments(event.target.checked)} />抓取全部评论</label><label className="inline-flex items-center gap-2"><input type="checkbox" checked={includeMedia} onChange={(event) => setIncludeMedia(event.target.checked)} />下载图片到本机</label></div><button className="button-primary mt-4 w-full" disabled={disabled || pids.length === 0}>同步 {new Set(pids).size || ''} 个 PID</button></form>
}
