...
function LoginPanel({ status, onStatus }: { status?: AuthStatus; onStatus: (value: AuthStatus) => void }) {
  ...
  const login = useMutation({ mutationFn: () => api.loginSession(username.trim(), password), onSuccess: (value) => { setPassword(''); onStatus(value) } })
  const challenge = status?.challenge === 'sms' || status?.challenge === 'otp' ? status.challenge : undefined
  const verify = useMutation({ mutationFn: () => api.continueSession(challenge!, code.trim()), onSuccess: (value) => { setCode(''); onStatus(value) } })
  if (challenge) return <form className="w-full max-w-md rounded-xl border border-line bg-white/55 p-4 lg:w-[420px]" onSubmit={(event) => { event.preventDefault(); if (code.trim()) verify.mutate() }}><p className="text-sm font-semibold">输入{challenge === 'sms' ? '短信验证码' : '动态口令'}</p><div className="mt-3 flex gap-2"><input className="field" value={code} onChange={(event) => setCode(event.target.value)} inputMode="numeric" autoComplete="one-time-code" placeholder="验证码" /><button className="button-primary shrink-0" disabled={!code.trim() || verify.isPending}>{verify.isPending ? '验证中…' : '继续登录'}</button></div>{verify.error && <p className="mt-2 text-xs text-coral">{verify.error.message}</p>}</form>
  return <form className="grid w-full max-w-md gap-3 rounded-xl border border-line bg-white/55 p-4 lg:w-[420px]" onSubmit={(event) => { event.preventDefault(); if (username.trim() && password) login.mutate() }}><input className="field" value={username} onChange={(event) => setUsername(event.target.value)} autoComplete="username" placeholder="北大学号" /><input className="field" type="password" value={password} onChange={(event) => setPassword(event.target.value)} autoComplete="current-password" placeholder="密码（不会由网页保存）" /><button className="button-primary" disabled={!username.trim() || !password || login.isPending}>{login.isPending ? '登录中…' : '登录并保存本机会话'}</button>{login.error && <p className="text-xs text-coral">{login.error.message}</p>}</form>
}
