...
function ObserverArchiveBanner({ status, error, refresh }: { status?: ObserverStatus; error: unknown; refresh: () => unknown }) {
	...
	if (!status.connected || status.challenge_required || status.stale || status.coverage_degraded) {
		const waitingForOTP = status.challenge_required && status.challenge === 'otp'
		return <div className="mb-5 flex items-start gap-3 rounded-xl border border-coral/30 bg-coral-soft/25 p-4" role="status"><AlertTriangle className="mt-0.5 shrink-0 text-coral" size={20} /><div><p className="font-semibold">{status.challenge_required ? waitingForOTP ? 'Observer 等待手机令牌' : 'Observer 等待短信验证' : !status.connected ? 'Observer 当前离线' : status.coverage_degraded ? 'Observer 扫描覆盖不足' : 'Observer 数据已经过期'}</p><p className="mt-1 text-xs leading-5 text-ink-soft">{status.last_error || (status.challenge_required ? waitingForOTP ? '前往设置提交北京大学 App 的 6 位动态口令；本地归档不受影响。' : '前往设置提交短信验证码；本地归档不受影响。' : '新的删除可能尚未同步，已有本地归档仍可正常查看。')}</p></div></div>
	}
