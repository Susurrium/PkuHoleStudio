...
## v0.1.0-alpha.7 - 2026-07-28

- 登录时通过 IAAA 的结构化认证模式区分短信验证码与北京大学 App 手机令牌；OTP 场景不再误导用户等待短信，并明确提示 6 位动态口令的获取位置。
- 热榜 API 增加 1–20 条的有界 `limit` 参数；Web 获取 Top 10、默认展示前五条并可展开完整榜单，TUI 继续保持五条紧凑视图。
- Studio 默认布局改进平板与移动端路径：固定侧栏延后到 1024px，本地详情的项目、标签和笔记移动到评论之前，并增加项目直接导出。
- 在线首页补充移动端热榜入口，“保存到本地”导航明确为“登录与保存”；Toolkit 外部入口收纳到可选传输页，首次引导的关闭语义与持久行为保持一致。
- 兼容 API 服务器默认仅监听 `127.0.0.1`；非回环监听必须显式使用 `--allow-remote` 并配置至少 24 字符的 Bearer Token。
- API 文档中的可识别成绩样例已替换为完全虚构数据；新增来源与分发边界 NOTICE，明确当前没有上游再分发授权记录。
